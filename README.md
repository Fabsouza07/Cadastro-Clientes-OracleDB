...
3. Configure `ORACLE_DB_URL`, `ORACLE_DB_USUARIO`, `ORACLE_DB_SENHA` e `ORACLE_DB_SCHEMA` como variáveis de ambiente. Use `CADASTRO_APP` para a aplicação e `CADASTRO_CLIENTES` como schema. Esses nomes evitam conflito com outras aplicações de banco, como MySQL. Como alternativa local, copie `src/main/resources/config.properties.example` para `config.properties` e preencha a senha. Esse arquivo não deve ser enviado ao Git.
   ...
   **Segurança:** mantenha `config.properties.example` no repositório como modelo apenas, e nunca versiona `config.properties` com credenciais reais. Use `config.properties` somente localmente e adicione-o ao `.gitignore` se ainda não estiver ignorado.
   ...
   ```powershell
   $env:ORACLE_DB_URL = 'jdbc:oracle:thin:@//localhost:1521/FREEPDB1'
   $env:ORACLE_DB_USUARIO = 'cadastro_app'
   $env:ORACLE_DB_SENHA = 'sua_senha_segura'
   $env:ORACLE_DB_SCHEMA = 'cadastro_clientes'
   ```
   ...
   export ORACLE_DB_SCHEMA='cadastro_clientes'
   ```
